...
import type { CustomerProfile, Order, OrderItem } from '@/lib/generated/prisma/client'
...
type OrderWithAll = Order & {
  items: OrderItem[]
  customer: { businessName: string; contactName: string }
}
...
export default function OrdersAdmin({ orders, customers, products }: Props) {
  ...
  const columns: Column<OrderWithAll>[] = [
    {
      key: 'id',
      header: 'Order ID',
      render: (row) => (
    ...
    },
    {
      ...
      header: 'Status',
      render: (row) => (
        <Badge variant={STATUS_VARIANT[row.status.toLowerCase()] ?? 'outline'}>
          {row.status}
        </Badge>
      ),
    },
  ...
  return (
    <div className="space-y-4">
      ...
      <Dialog
        ...
        onOpenChange={(open) => {
          setDialogOpen(open)
          if (!open) { setLoyaltyNote(null); setError(null) }
        }}
      >
        <DialogContent>
          <DialogHeader>
            ...
            <div className="grid gap-3 py-2">
              ...
              <div className="grid gap-1">
                ...
                <Select
                  ...
                  onValueChange={(v) => { if (v) setCustomerIdSelect(v) }}
                >
              ...
              <div className="grid gap-1">
                <Label>Product</Label>
                <input type="hidden" name="productId" value={productIdSelect} />
                <Select
                  value={productIdSelect}
                  onValueChange={(v) => { if (v) setProductIdSelect(v) }}
                >
                  <SelectTrigger className="w-full">
